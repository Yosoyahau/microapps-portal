  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-8 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-base-content to-base-content/70 bg-clip-text text-transparent">{title}</h1>
          <p className="text-base-content/50 mt-1">{subtitle}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {stats.map((stat, i) => (
          <GlassCard key={i} className="p-5 flex flex-col justify-between hover:bg-base-content/[0.04] transition-colors group">
            <div className="flex items-start justify-between mb-4">
              <div className={`p-2.5 rounded-xl bg-base-content/5 border border-base-content/10 group-hover:bg-base-content/10 transition-colors ${stat.color}`}>
                <stat.icon className="w-5 h-5" />
              </div>
              <div className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full bg-black/20 border border-base-content/5 ${stat.isUp ? 'text-green-400' : 'text-red-400'}`}>
                {stat.isUp ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
                {stat.change}
              </div>
            </div>
            <div>
              <p className="text-sm font-medium text-base-content/50 mb-1">{stat.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <GlassCard className="lg:col-span-2 p-6 flex flex-col">
          <div className="flex items-center justify-between mb-8 shrink-0">
            <h2 className="text-lg font-semibold text-base-content">{chartActivity}</h2>
          ...
          {/* Pure CSS Bar Chart */}
          <div className="h-48 sm:h-64 grid grid-cols-7 gap-2 sm:gap-6 pt-4 pb-8 border-b border-base-content/10 relative mt-auto">
            {/* Y-axis grid lines (decorative) */}
            <div className="absolute inset-x-0 bottom-1/3 border-t border-base-content/5 pointer-events-none w-full" />
            <div className="absolute inset-x-0 bottom-2/3 border-t border-base-content/5 pointer-events-none w-full" />
            ...
            {barHeights.map((h, i) => (
              <div key={i} className="relative h-full group">
                ...
                <span className="absolute -bottom-7 left-1/2 -translate-x-1/2 text-xs text-base-content/40 group-hover:text-base-content/70 transition-colors">{barLabels[i]}</span>
              </div>
            ))}
          </div>
        </GlassCard>
        ...
        <div className="space-y-6 flex flex-col">
          ...
          <GlassCard className="p-6 shrink-0">
            <h2 className="text-lg font-semibold text-base-content mb-4">{quickActions}</h2>
            <div className="space-y-3">
              <button className="w-full flex items-center justify-between p-3 rounded-xl bg-base-content/5 hover:bg-base-content/10 border border-base-content/5 hover:border-base-content/10 transition-all group">
                <span className="text-sm font-medium text-base-content/80 group-hover:text-base-content">{actionReport}</span>
                <ArrowRight className="w-4 h-4 text-base-content/40 group-hover:text-base-content transition-colors" />
              </button>
              <button className="w-full flex items-center justify-between p-3 rounded-xl bg-base-content/5 hover:bg-base-content/10 border border-base-content/5 hover:border-base-content/10 transition-all group">
                <span className="text-sm font-medium text-base-content/80 group-hover:text-base-content">{actionInvite}</span>
                <ArrowRight className="w-4 h-4 text-base-content/40 group-hover:text-base-content transition-colors" />
              </button>
            </div>
          </GlassCard>
          ...
          <GlassCard className="p-6 flex-1 flex flex-col min-h-[250px]">
            <h2 className="text-lg font-semibold text-base-content mb-6 shrink-0">{recentAct}</h2>
            <div className="space-y-5 flex-1">
              {timeline.map((item) => (
                <div key={item.id} className="flex gap-4 relative">
                  {/* Timeline connecting line */}
                  {item.id !== timeline.length && (
                    <div className="absolute left-4 top-10 bottom-[-20px] w-px bg-base-content/10" />
                  )}
                  
                  <div className="w-8 h-8 rounded-full bg-base-content/5 border border-base-content/10 flex items-center justify-center shrink-0 z-10 text-base-content/50">
                    <Clock className="w-4 h-4" />
                  </div>
                  
                  <div className="flex-1 pb-1">
                    <p className="text-sm text-base-content/80">
                      <span className="font-semibold text-base-content">{item.user}</span> {item.action}
                    </p>
